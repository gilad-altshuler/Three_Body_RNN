...
# ---------- run ----------
mkdir -p "$ROOT/outputs/reach_inference/reach_condition"
PIDFILE="$ROOT/outputs/reach_inference/reach_condition/pids.txt"
: > "$PIDFILE"
...
(
  ...
  for i in $(seq "$START" "$((START + N - 1))"); do
    for cfg in "${CONFIGS[@]}"; do
      ...
      cmd=(env
           OVERLAY="$OVERLAY" REPO="$REPO" TRAIN_SCRIPT="reach_condition"
           RNN_IMPL="$MODEL"
...
python "$ROOT/training_scripts/reach_inference/reach_condition/collect_data.py"

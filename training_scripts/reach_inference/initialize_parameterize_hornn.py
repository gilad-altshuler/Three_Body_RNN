...
def initialize_Ws_tbrnn(dz, N):
    l = nn.Parameter(nn.init.xavier_uniform_(torch.empty((N,dz),requires_grad=True)))
    m = nn.Parameter(nn.init.xavier_uniform_(torch.empty((dz,N),requires_grad=True)))
    n = nn.Parameter(nn.init.xavier_uniform_(torch.empty((dz,N),requires_grad=True)))
    return l, m, n

def initialize_Ws_rnn(dz, N):
    m = nn.Parameter(nn.init.xavier_uniform_(torch.empty((N,dz),requires_grad=True)))
    n = nn.Parameter(nn.init.xavier_uniform_(torch.empty((dz,N),requires_grad=True)))
    return m, n

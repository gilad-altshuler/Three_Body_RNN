...
(
  ...
  LOG_DIR="$ROOT/outputs/reach_inference/hornn/r_4_r_1"
  mkdir -p "$LOG_DIR"
  ...
  for i in $(seq "$START" "$((START + N - 1))"); do
    ...
    RNN_IMPL=hornn RNN_DIM=4 TBRNN_DIM=1 \
    PYTHONPATH="$OVERLAY:$REPO:$PYTHONPATH" \
    nohup conda run -n smc_rnn_env \
      python -m train_scripts.macaque_reach.train_single_conditioning \
      --run_name "reach_conditioning/r_4_r_1_hornn/$RUN" \
